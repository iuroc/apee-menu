 ...
 */
export function removeSubMenu(menuId?: string) {
    if (typeof menuId != 'string') menuId = '0'
    /** 父菜单 ID */
 ...
 */
export function makeNewMenu(menuConfig: Menu, left: number, top: number, menuId: string = '0') {
    // 如果没有菜单项，则不生成菜单
    ...
    menuConfig.forEach(itemConfig => {
        ...
        if (hasSubMenu) {
            ...
            itemEle.addEventListener('mouseenter', () => {
                ...
                makeNewMenu(itemConfig.subMenu as Menu, left, top, newItemId)
            })
        } else {
            itemEle.addEventListener('mouseenter', () => {
                removeSubMenu(menuId)
            ...
            itemEle.addEventListener('click', () => {
                if (itemConfig.autoClose || itemConfig.autoClose == undefined) removeSubMenu()
